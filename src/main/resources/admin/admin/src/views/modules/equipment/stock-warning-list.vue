                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('equipment','导入导出')"
                           icon="el-icon-download"
                           href="https://localhost:8443/equipmentzulinxitong/upload/equipmentMuBan.xls"
                        >批量导入设备数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('equipment','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="equipmentUploadSuccess"
                                :on-error="equipmentUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('equipment','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "equipment.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="equipmentUuidNumber"
                                   header-align="center"
                                   label="设备编号">
                        <template slot-scope="scope">
                            {{scope.row.equipmentUuidNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="设备库存">
                        <template slot-scope="scope">
                            {{scope.row.equipmentKucunNumber}}
                        </template>
                    </el-table-column>
...
<!-- 库存 -->
		<el-dialog
                :title="equipmentKucunNumberTitle"
                :visible.sync="equipmentKucunNumberVisible"
                width="30%">
            <span>操作数量</span><el-input-number v-model="kucunNumber" :min="1" :max="100" label="描述文字"></el-input-number>
            <span slot="footer" class="dialog-footer">
			<el-button @click="equipmentKucunNumberVisible = false">取 消</el-button>
			<el-button type="primary" @click="addEquipmentKucunNumber()">确 定</el-button>
		  </span>
        </el-dialog>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                id : null,
                equipmentUuidNumber : null,
                equipmentName : null,
                equipmentPhoto : null,
                equipmentTypes : null,
                equipmentKucunNumber : null,
                equipmentNewMoney : null,
                equipmentClicknum : null,
        ...
        //库存
            ...
            //原有数量
            equipmentKucunNumber: 0,
            //标题
            equipmentKucunNumberTitle: null,
            //模态框状态
            equipmentKucunNumberVisible: false,
            };
        },
        created() {
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'equipment',//当前表表名,
                        sumColum : 'equipment_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'equipment_name',//分组字符串字段
                        // types : 'equipment_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                params['equipmentDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "equipmentStockWarning/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            equipmentUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "equipment/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            equipmentUploadError(data){
                this.$message.error('上传失败');
            },
            //增加设备库存
			plusEquipmentKucunNumber(thisId,equipmentKucunNumber){
			//清空数据
				//设备库存数据设置为0
				this.kucunNumber = 0
				//id清空
			...
			//数据不为空放值
				...
				//将模态框打开
				this.equipmentKucunNumberVisible = true;
				//标题
				this.equipmentKucunNumberTitle = '增加设备库存'
			},
			//减少设备库存
			reduceEquipmentKucunNumber(thisId,equipmentKucunNumber){
			//清空数据
				//设备库存数据设置为0
				this.kucunNumber = 0
				//id清空
			...
			//数据不为空放值
				...
				this.equipmentKucunNumberVisible = true;
				//标题
				this.equipmentKucunNumberTitle = '减少设备库存'
			},
			//提交用户操作设备库存数量
			addEquipmentKucunNumber(){
				var type = null
				var maxNumber = 0
				if(this.kucunNumberTypes == "+"){
					type = '增加'
					maxNumber = this.equipmentKucunNumber + this.kucunNumber
				}else{
					type = '减少'
					if(this.equipmentKucunNumber < this.kucunNumber || this.kucunNumber <= 0){
						this.$message.error('设备库存不足不能减少');
						return false;
					}else{
						maxNumber = this.equipmentKucunNumber - this.kucunNumber
					}
				...
				let date = {
					...
					"equipmentKucunNumber":maxNumber
				}
				...
				this.equipmentKucunNumberVisible = false;
			},	
			//上下架
