        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        <el-button
                                v-if="isAuth('equipmentCommentback','新增')"
                                type="success"
                                icon="el-icon-plus"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('equipmentCommentback','删除')"
                        ...
                        &nbsp;
                        <el-button
                                v-if="isAuth('equipmentCommentback','报表')"
                                type="success"
                        ...
                        <el-upload
                                ...
                                :action="getActionUrl"
                                :on-success="equipmentCommentbackUploadSuccess"
                                :on-error="equipmentCommentbackUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('equipmentCommentback','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入设备评价数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('equipmentCommentback','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "equipmentCommentback.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                          ...
                          v-if="isAuth('equipmentCommentback','查看')"
                          :data="dataList"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="equipmentCommentbackText"
                                      header-align="center"
                                      label="评价内容">
                        <template slot-scope="scope">
                            <span v-if="scope.row.equipmentCommentbackText != null &&scope.row.equipmentCommentbackText.length>10">
                                {{scope.row.equipmentCommentbackText.slice(0,10)}}...
                            </span>
                            <span v-else>
                                {{scope.row.equipmentCommentbackText}}
                            </span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('equipmentCommentback','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('equipmentCommentback','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">回复</el-button>
                            <el-button v-if="isAuth('equipmentCommentback','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'equipment_commentback',//当前表表名,
                        sumColum : 'equipment_commentback_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'equipment_commentback_name',//分组字符串字段
                        // types : 'equipment_commentback_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                //         groupColumn: "equipment_commentback_types",
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                params['equipmentCommentbackDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "equipmentCommentback/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
            ...
            deleteHandler(id) {
                ...
                this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
                    ...
                    this.$http({
                        url: "equipmentCommentback/delete",
                        method: "post",
                        data: ids
                    }).then(({data}) => {
                        if(data && data.code === 0){
            ...
            // 导入功能上传文件成功后调用导入方法
            equipmentCommentbackUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "equipmentCommentback/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
            ...
            // 导入功能上传文件失败后调用导入方法
            equipmentCommentbackUploadError(data){
            ...
            wuyong(id) {
                ...
                _this.$confirm(`确定    操作?`, "提示", {confirmButtonText: "确定",cancelButtonText: "取消",type: "warning"
                    ...
                    _this.$http({
                        url: "equipmentCommentback/update",
                        method: "post",
                        data: {
                            id:id,
//                            equipmentCommentbackTypes:equipmentCommentbackTypes,
                        }
                    }).then(({data}) => {
                        if(data && data.code === 0){
